...
function removeFirstOccurrences(str, value) {
  const firstOccurIndex = str.indexOf(value);
  if (firstOccurIndex === -1) {
    return str;
  }
  return str
    .substring(0, firstOccurIndex)
    .concat(str.substring(firstOccurIndex + value.length));
}
...
function removeLastOccurrences(str, value) {
  const firstOccurIndex = str.lastIndexOf(value);
  if (firstOccurIndex === -1) {
    return str;
  }
  return str
...
function formatTime(minutes, seconds) {
  ...
  const formatedSesonds = seconds.toString().padStart(2, '0');
  return `${formatedMinutes}:${formatedSesonds}`;
}
...
function countVowels(str) {
  const regexp = /[a, e, i, o, u, y, A, E, I, O, U, Y]/g;
...
function isPalindrome(str) {
  const regexp = /[a-z]/gi;
  const onlyLettersStr = [...str.matchAll(regexp)].join('').toLowerCase();
  const onlyLettersStrReverse = reverseString(onlyLettersStr).toLowerCase();
  return onlyLettersStr === onlyLettersStrReverse;
}
